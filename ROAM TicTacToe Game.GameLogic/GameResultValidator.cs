    ...
    public class GameResultValidator : IGameResultValidator
    {
        ...
        private char GetWinner(string board)
        {
            ...
            foreach (var win in wins)
            {
                ...
                {
                    if (winner != '?')
                        throw new InvalidOperationException("More than 1 winning line on the board!");
                    winner = p;
                }
            }
        ...
        {
            if (board.Length != 9)
                throw new InvalidOperationException("Board has invalid number of characters!");

            if (!board.All(c => ValidCharacters.Contains(c)))
                throw new InvalidOperationException("Board has invalid characters!");

            int countX = board.Count(c => c == 'X');
            int countO = board.Count(c => c == 'O');
            if (Math.Abs(countX - countO) > 1)
                throw new InvalidOperationException("Board has invalid number of turns!");
        }
    }
}